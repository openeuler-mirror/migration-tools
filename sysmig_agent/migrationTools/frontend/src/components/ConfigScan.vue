...
<el-main>
    <div>
        ...
        <el-row class="infoItemLineMargin">
            ...
            <el-col :span="18" style="margin-top:5px">{{confScanResult.arch}}</el-col>
        </el-row>
        <el-row class="infoItemLineMargin">
            <el-col :span="6" class="infoItemTitle">迁移类型:</el-col>
            <el-col :span="18" style="margin-top:5px">{{confScanResult.desc}}</el-col>
        </el-row>
        </div>
    </div>
        ...
        <el-table :data="onShowConfList" style="width: 100%">
            ...
            <el-table-column label="兼容性" width="130px" align="center">
                <template #default="scope">
                    <div class="pkgTag"
                            :class="{'itemTagSame': scope.row.res=='same',
                            'itemTagDifferent': scope.row.res=='different'}">{{ scope.row.res }}</div>
                </template>
            </el-table-column>
        </el-table>
    </div>
    <el-button type="primary" class="loadMoreButton" @click="loadAllConfList()" v-if="!isShowingAllConf">点击加载剩余数据</el-button>
    <el-button type="primary" class="loadMoreButton" @click="foldConfList()" v-if="isShowingAllConf">点击折叠列表</el-button>
    ...
    <el-button type="primary" class="loadMoreButton" @click="loadAllServiceList()" v-if="!isShowingAllService">点击加载剩余数据</el-button>
    <el-button type="primary" class="loadMoreButton" @click="foldServiceList()" v-if="isShowingAllService">点击折叠列表</el-button>
