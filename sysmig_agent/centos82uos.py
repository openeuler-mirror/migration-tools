...
def pre_system_rpms_info():
    ...
    out2 = subprocess.check_output(
        'rpm -Va | sort -k3 > "/var/tmp/uos-migration/UOS_migration_log/rpms-verified-before.txt"', shell=True)
...
def centos8_main(osname,task_id):
    ...
    migration_log.info("Checking if the tool is executed by root user")
    badpackages = get_bad_packages()
    # check required packages
    migration_log.info('Checking required packages')
    for pkg in ['rpm', 'yum', 'curl']:
        if not check_pkg(pkg):
            migration_log.info("Could not found " + pkg)
            sql_mig_statue('28')
            sql_task_statue(3)
    # sto system rpms info before migration
    pre_system_rpms_info()
    # check if the os old_version is supported
    migration_log.info("========= Checking: distribution =========")
    ...
    if len(old_version) == 0:
        migration_log.info("You appear to be running an unsupported distribution.")
        sql_mig_statue('28')
        sql_task_statue(3)
    if len(old_version) > 1:
        migration_log.info("Could not determine your distribution because multiple packages are providing redhat-release:")
        migration_log.info('\n'.join(old_version))
        sql_mig_statue('28')
        sql_task_statue(3)

    old_version = old_version[0]
    migration_log.info(old_version)
    osrelease = osname + '-release'
    linux_release = osname + '-linux-release'
    if re.match('uos-release', old_version):
        migration_log.info("You are already using Uniontech.")
        sql_mig_statue('02')
        ...
        migration_log.info("Your are using an unsupported distribution.")
        sql_mig_statue('28')
        sql_task_statue(3)
    if re.match('7', subver):
        migration_log.info("Your system will be migrated from CentOS7 to UniontechOS")
    elif re.match('20', subver):
        migration_log.info("You appear to be running an unsupported distribution.")
        sql_mig_statue('02')

    migration_log.info("========= Checking: required python packages =========")
    if not check_pkg('/usr/libexec/platform-python'):
        migration_log.info('/usr/libexec/platform-python not found.')
        sql_mig_statue('28')
        sql_task_statue(3)
    base_packages = ['basesystem', 'initscripts', 'uos-logos', 'plymouth', 'grub2', 'grubby']

    migration_log.info("========= Checking: yum lock ===========")
    if os.path.exists('/var/run/yum.pid'):
        with open('/var/run/yum.pid', 'r') as f:
            ...
            with open('/proc/' + pid + '/comm', 'r') as ff:
                comm = ff.read()
                migration_log.info('Another app is currently holding the yum lock: ' + comm)
                migration_log.info('Running as pid: ' + pid)
                migration_log.info('Please kill it and run the tool again.')
                ff.close()
            sql_mig_statue('18')
            #sql_task_statue(3)
    ...
    if re.match('8\.', subver):
        migration_log.info("========= Checking: dnf =========")
        migration_log.info("Identifying dnf modules that are enabled...")
        enabled_modules = str(
        ...
        if len(enabled_modules) > 0:
            ...
            if len(unknown_mods) > 0:
                migration_log.info('This tool is unable to automatically switch module(s) ' \
                            + ','.join(unknown_mods) \
    ...
    migration_log.info("========= Looking for yumdownloader ==========")
    ...
    migration_log.info("========= Start converting =========")
    subprocess.run('rpm -e --nodeps {}'.format(osrelease) , shell=True)
    ...
    migration_log.info("Downloading uos release package...")
    ...
    try:
        ...
        if not check_pkg('yumdownloader'):
            subprocess.run("yum -y install yum-utils --disablerepo C* || true", shell=True)
        if not check_pkg('yumdownloader'):
            migration_log.error("yumdownloader can not installe")
        pass
    except Exception:
        migration_log.info('\n'.join(dst_release))
        migration_log.info("Are you behind a proxy? If so, make sure the 'http_proxy' environmen")
        migration_log.info("variable is set with your proxy address.")
        migration_log.info("An error occurred while attempting to switch this system to Uniontech" + \
                    "and it may be in an unstable/unbootable state. To avoid further issues, " + \
                    "the script has terminated.")

    migration_log.info("Switching old release package with Uniontech...")
    ...
    migration_log.info("Installing base packages for UniontechOS...")
    cmd = 'yum shell -y <<EOF\n\
remove ' + badpackages + '\n\
...
EOF'
    ...
    if code != 0:
        migration_log.info("Could not install base packages.Run 'yum distro-sync' to manually install them...28.")
        sql_mig_statue('38')
        #sql_task_statue(3)
        sql_task_statue(3, task_id)
        return None
    ...
    if os.access('/usr/libexec/plymouth/plymouth-update-initrd', os.X_OK):
        migration_log.info("Updating initrd...")
        indexhtml = 'rpm -e --nodeps ' + osname + '-indexhtml'
        run_subprocess(indexhtml)
        ...
        fdout.close()
    migration_log.info("Switch successful. Syncing with UniontechOS repositories.")
    if subver == '8.3':
