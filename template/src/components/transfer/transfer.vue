<template>
  <div class="box">
    ...
    <el-card class="center">
      ...
      <el-table
        ref="multipleTable"
        :data="transData.slice((currentPage - 1) * pagesize, currentPage * pagesize)"
        tooltip-effect="dark"
        style="width: 100%"
        ...
        <el-table-column label="操作" width="300" align="center">
          <template slot-scope="scope">
            <el-button
              ...
              icon="el-icon-tickets" size="small"
              >迁移日志</el-button
            >
            <el-button
              type="primary"
    ...
    <div class="footer">
      <div>
        <el-row>
          <el-button @click="gohostment()"  :disabled="notempty(this.transData)">返回</el-button>
        </el-row>
      </div>
    </div>
  </div>
         ...
         console.log(this.transData1.agent_ip)
              for (var key in this.transData1) {
                  var info=[
                    ...
                    console.log(info)
                 this.$http.post("/system_migration",{
                        mod:this.mod2,
                        info: info
                    }).then((res)=>{
                        console.log(res)
          ...
          for (var key in this.transData) {
            ...
            this.transData1.map((item) => {
              ...
              if (this.transData[key].agent_ip == item.agent_ip) {
                ...
                this.transData[key].agent_online_status =item.agent_online_status;
