...
def check_user():
    ...
    for value in data:
        if value[1] == "root":
            ...
            sudo_v = "'echo %s | sudo -S -v'" % str(value[2], encoding="utf-8")
        check_sudo_v = "sshpass -p %s ssh %s@%s " % (str(value[2], encoding="utf-8"), value[1], value[0]) + sudo_v
        check_res = os.system(check_sudo_v)
        if check_res == 0:
...
def agent_rpm_issued(data):
    ...
    if data[1] == 'root':
        ...
        install_rpm = "sshpass -p %s ssh %s@%s 'echo %s |sudo -S yum install -y uos-sysmig-agent'" % (str(data[2], encoding="utf-8"), data[1], data[0], str(data[2], encoding="utf-8"))
        uos_sysmig_conf = "sshpass -p %s scp -r /etc/uos-sysmig/uos-sysmig.conf %s@%s:/etc/uos-sysmig/" % (str(data[2], encoding="utf-8"), data[1], data[0])
        restart_server = "sshpass -p %s ssh %s@%s 'echo %s |sudo -S systemctl restart uos-sysmig-agent'" % (str(data[2], encoding="utf-8"), data[1], data[0], str(data[2], encoding="utf-8"))
        agent = os.system(install_rpm)
        cp_uos_sysmig_conf = os.system(uos_sysmig_conf)
        restart_server = os.system(restart_server)
